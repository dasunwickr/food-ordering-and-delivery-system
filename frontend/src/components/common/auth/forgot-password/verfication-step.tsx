...
export function VerificationStep({
  ...
  // Update verification code when individual digits change
  useEffect(() => {
    updateVerificationCode(individualDigits.join(""))
  }, [individualDigits, updateVerificationCode])

  // Initialize individual digits from verification code
  useEffect(() => {
    if (verificationCode && individualDigits.join("") !== verificationCode) {
      setIndividualDigits(verificationCode.split("").concat(Array(6 - verificationCode.length).fill("")))
    }
  }, [verificationCode, individualDigits])
